...
> **Domain migration note (2026-05-05):** Backend `src/` đã refactor từ by-type folders (`api/`, `models/`, `services/`, `repositories/`, `schemas/`, `base/`) sang by-domain folders (`auth/`, `itineraries/`, `places/`, `shared/`, `core/`). Các path reference dưới đây vẫn dùng path cũ vì là historical; code thực tế giờ nằm trong domain packages. Xem PRs #34–#38.

...
## Phần 8: API Endpoints — Từ 10 lên 34 core

FE mới có 25 trang, mỗi trang cần API riêng. Đây là **25 endpoint MỚI** cần xây trong MVP2 core (21 gốc + EP-31 Forgot Password + EP-32 Reset Password + EP-34 Guest Claim + EP-35 Chat History), nhóm theo chức năng. `EP-36 /agent/analytics` nằm trong docs nhưng là optional/MVP2+ vì cần hardening Text-to-SQL.

> **EP numbering update (2026-05-04):** EP-31/32 code = forgot/reset-password (PR #20).
> Phase C endpoints renumbered: EP-33 rate-limit-status, EP-34 claim, EP-35 chat-history, EP-36 analytics.
> Tổng core: 34 endpoints.

### Nhóm Auth (thêm 4 endpoint mới)
- `POST /auth/refresh` — Gia hạn token không cần login lại
- `POST /auth/logout` — Logout thật, revoke token trên server
- `POST /auth/forgot-password` — Gửi email reset password (EP-31, PR #20)
- `POST /auth/reset-password` — Tiêu hau reset token + đổi mật khẩu (EP-32, PR #20)
...
### Nhóm AI Agent (5 endpoint hoàn toàn mới)
...
- `GET /agent/rate-limit-status` — Kiểm tra còn bao nhiêu lượt AI hôm nay (EP-33)
- `GET /agent/chat-history/{trip_id}` — Lịch sử chat AI (EP-35)

> 📖 Bảng mapping đầy đủ 34 core endpoints + EP-36 optional → function: [implementation_plan.md](implementation_plan.md) Endpoint Table

# Part 12: BE CRUD Endpoints — Bảng tổng hợp 34 core API + EP-36 optional

> **Domain migration note (2026-05-05):** Backend `src/` đã refactor từ by-type folders sang by-domain folders (`auth/`, `itineraries/`, `places/`, `shared/`, `core/`). File paths trong bảng "File nào implement" vẫn dùng path cũ vì là historical; code thực tế giờ nằm trong domain packages. Xem PRs #34–#38.
...
### EP-31: `POST /api/v1/auth/forgot-password`

| | |
|---|---|
| **Mô tả** | Gửi email reset password (silent nếu email không tồn tại) |
| **Auth** | ❌ Public |
| **Rate limit** | Không |
...
**How it works:**
1. Router parse `ForgotPasswordRequest` (Pydantic validate email format)
2. Service: `UserRepo.get_by_email(email)` → nếu không tồn tại → vẫn return success (chống email enumeration)
3. Service: tạo opaque reset token (`secrets.token_urlsafe(32)`)
4. Service: hash reset token (SHA-256), lưu `password_reset_token_hash` + `password_reset_expires_at` vào `users` row
5. Service: gửi email qua `EmailService` chứa reset link `FRONTEND_URL/reset-password?token={raw_token}`
6. Return success response (luôn giống nhau dù email tồn tại hay không)

**Security rules:**
- Reset token lưu **hash** trong DB (giống refresh/share/claim tokens), KHÔNG lưu raw token
- Token **one-time use**: sau khi dùng, hash + expiry bị clear
- Token có thời hạn (mặc định 1 giờ, cấu hình `PASSWORD_RESET_TOKEN_EXPIRE_HOURS`)
- `forgot-password` luôn trả success dù email không tồn tại (chống email enumeration)
- `smtp_password` là `SecretStr`, chỉ set qua `.env`, không qua `config.yaml`

**Implementation chain:**
```
Router: src/api/v1/auth.py → forgot_password()
Service: src/services/auth_service.py → AuthService.forgot_password()
Repo:    src/repositories/user_repo.py → UserRepo.get_by_email(), update_reset_token()
Email:   src/services/email_service.py → EmailService.send_reset_email()
Schema:  src/schemas/auth.py → ForgotPasswordRequest
DB:      users (UPDATE password_reset_token_hash, password_reset_expires_at)
Test:    TC-AUTH-05 (success), TC-AUTH-06 (email not found — same response)
```

---

### EP-32: `POST /api/v1/auth/reset-password`

| | |
|---|---|
| **Mô tả** | Tiêu hao reset token + đổi mật khẩu mới |
| **Auth** | ❌ Public |
| **Rate limit** | Không |
...
|-------|------|------------|
| `token` | `string` | Required, opaque reset token từ email |
...
| Status | error_code | Khi nào |
|--------|------------|---------|
| 400 | `BAD_REQUEST` | Token không hợp lệ, đã hết hạn, hoặc đã được sử dụng |
| 422 | `VALIDATION_ERROR` | newPassword < 6 ký tự |

**How it works:**
1. Router parse `ResetPasswordRequest` (Pydantic validate)
2. Service: hash `token` → `UserRepo.get_by_reset_token_hash(hashed_token)` → nếu không tìm thấy → 400
3. Service: check `password_reset_expires_at` → nếu đã hết hạn → 400
4. Service: `bcrypt.hash(newPassword)` → update `hashed_password`, clear `password_reset_token_hash` + `password_reset_expires_at`
5. Service: **revoke tất cả refresh tokens** cho user (force re-login trên mọi thiết bị)
6. Return success response

**Implementation chain:**
```
Router: src/api/v1/auth.py → reset_password()
Service: src/services/auth_service.py → AuthService.reset_password()
Repo:    src/repositories/user_repo.py → UserRepo.get_by_reset_token_hash(), clear_reset_token()
         src/repositories/token_repo.py → RefreshTokenRepo.revoke_all()
Schema:  src/schemas/auth.py → ResetPasswordRequest
DB:      users (UPDATE hashed_password, CLEAR reset token fields), refresh_tokens (UPDATE is_revoked)
Test:    TC-AUTH-07 (success), TC-AUTH-08 (expired token), TC-AUTH-09 (already used token)
```

> **Note EP numbering (2026-05-04):** EP-31 và EP-32 gốc plan lần lượt là `GET /agent/rate-limit-status`
> và `POST /itineraries/{id}/claim`. Code thực tế assign EP-31 cho forgot-password và EP-32 cho
> reset-password (PR #20). Guest claim đã nằm trong itineraries router (không có số EP riêng).
> Rate-limit-status sẽ là EP-33 khi implement Phase C; chat-history sẽ là EP-34; analytics sẽ là EP-35.
> Tổng core sau Phase C: **34 endpoints** (27 gốc + 3 AI + 2 auth mới + rate-limit + chat-history).

---

### EP-33: `GET /api/v1/agent/rate-limit-status`

> **Renumbered:** EP-31 cũ → EP-33 mới, do EP-31/EP-32 đã assign cho forgot/reset-password.
...
### EP-34: `POST /api/v1/itineraries/{id}/claim`

> **Renumbered:** EP-32 cũ → EP-34 mới. Claim endpoint đã implement trong itineraries router.
...
### EP-35: `GET /api/v1/agent/chat-history/{trip_id}`

> **Renumbered:** EP-33 cũ → EP-35 mới.
...
| 31 | POST | `/auth/forgot-password` | ❌ | 200 | `auth.py` | 🆕 PR #20
| 32 | POST | `/auth/reset-password` | ❌ | 200 | `auth.py` | 🆕 PR #20
| 33 | GET | `/agent/rate-limit-status` | 🔒 | 200 | `agent.py` | Phase C
| **34** | **POST** | **`/itineraries/{id}/claim`** | **🔒** | **200** | **`itinerary.py`** | ✅ implemented
| **35** | **GET** | **`/agent/chat-history/{trip_id}`** | **🔒** | **200** | **`agent.py`** | Phase C
| **36** | **POST** | **`/agent/analytics`** | **🔒** | **200** | **`agent.py`** | 🆕 optional/MVP2+

**Legend:** ❌ Public | 🔓 Optional (guest OK) | 🔒 Required (JWT)

> [!IMPORTANT]
> Tổng core: **34 endpoints** (27 gốc + 3 AI + 2 auth mới EP-31/32 + EP-33 rate-limit + EP-35 chat-history).
> EP-34 claim đã implement trong itineraries router. EP-36 analytics là optional/MVP2+.
> `GET /shared/{shareToken}` là public read path của EP-15 share flow.
>
> **EP numbering update (2026-05-04):** EP-31/32 gốc plan (rate-limit-status, claim) đã được
> reassign cho forgot-password/reset-password (PR #20). Phase C endpoints dịch lên: EP-33 rate-limit-status,
> EP-34 claim, EP-35 chat-history, EP-36 analytics. Xem chi tiết tại note trước EP-31 section.

---

## EP-36: POST /agent/analytics — Text-to-SQL Analytics 🆕 optional/MVP2+

> **Renumbered:** EP-34 cũ → EP-36 mới.

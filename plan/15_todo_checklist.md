...
> **Domain migration note (2026-05-05):** Backend `src/` đã refactor từ by-type folders (`api/`, `models/`, `services/`, `repositories/`, `schemas/`, `base/`) sang by-domain folders (`auth/`, `itineraries/`, `places/`, `shared/`, `core/`). Phase A–B3 completed tasks vẫn ghi path cũ vì là historical record; Phase C planned paths đã update sang domain convention. Xem PRs #34–#38.
...
### B1-EXT: Password Reset (EP-31, EP-32) — PR #20

- [x] **B1-EXT.1** — Alembic migration `20260504_0003`: thêm `password_reset_token_hash`, `password_reset_expires_at` vào `users`
  - Files: `alembic/versions/20260504_0003_password_reset.py`
  - Endpoint: EP-31, EP-32
  - Dependency: A4.1
  - Estimate: small
  - Done: Migration tạo 2 column nullable mới trong users table

- [x] **B1-EXT.2** — `User` model: thêm 2 field mới
  - Files: `src/models/user.py` (modify)
  - Dependency: B1-EXT.1
  - Estimate: small
  - Done: `password_reset_token_hash`, `password_reset_expires_at` mapped columns

- [x] **B1-EXT.3** — `security.py`: thêm `create_password_reset_token()` utility
  - Files: `src/core/security.py` (modify)
  - Dependency: A2.3
  - Estimate: small
  - Done: Generate opaque token + SHA-256 hash

- [x] **B1-EXT.4** — `config.py`: thêm `password_reset_token_expire_hours`, SMTP config
  - Files: `src/core/config.py` (modify), `config.yaml` (modify)
  - Dependency: A2.1
  - Estimate: small
  - Done: New config params: `password_reset_token_expire_hours`, `smtp_host`, `smtp_port`, `smtp_username`, `smtp_password` (SecretStr), `email_from_address`

- [x] **B1-EXT.5** — `schemas/auth.py`: thêm `ForgotPasswordRequest`, `ResetPasswordRequest`
  - Files: `src/schemas/auth.py` (modify)
  - Dependency: A5.1
  - Estimate: small
  - Done: Pydantic schemas cho EP-31, EP-32

- [x] **B1-EXT.6** — `email_service.py`: gửi email qua `aiosmtplib`, fallback log-to-console
  - Files: `src/services/email_service.py` (new)
  - Endpoint: EP-31
  - Dependency: B1-EXT.4
  - Estimate: medium
  - Done: SMTP mode khi `smtp_host` có giá trị; Console mode khi trống. `aiosmtplib` dependency thêm vào `pyproject.toml`

- [x] **B1-EXT.7** — `auth_service.py`: thêm `forgot_password()`, `reset_password()` methods
  - Files: `src/services/auth_service.py` (modify)
  - Endpoint: EP-31, EP-32
  - Dependency: B1.3, B1-EXT.6
  - Estimate: medium
  - Done: forgot_password → create token + email; reset_password → verify hash + change password + revoke all refresh tokens

- [x] **B1-EXT.8** — `user_repo.py`: thêm `get_by_reset_token_hash()`
  - Files: `src/repositories/user_repo.py` (modify)
  - Endpoint: EP-32
  - Dependency: B1.1
  - Estimate: small
  - Done: Query user by SHA-256 hash of reset token

- [x] **B1-EXT.9** — `auth.py` router: thêm EP-31 forgot-password, EP-32 reset-password
  - Files: `src/api/v1/auth.py` (modify)
  - Endpoint: EP-31 `POST /auth/forgot-password`, EP-32 `POST /auth/reset-password`
  - Dependency: B1-EXT.7
  - Estimate: medium
  - Done: 2 new auth endpoints

- [x] **B1-EXT.10** — 7 unit tests cho password reset flow
  - Files: `tests/unit/test_password_reset.py` (new)
  - Dependency: B1-EXT.7
  - Estimate: medium
  - Done: Test forgot_password (success, email not found), reset_password (success, expired, already used, invalid)

- [x] **B1-EXT.11** — FE: `services/auth.ts` thêm `forgotPassword()`, `resetPassword()`
  - Files: `Frontend/src/app/services/auth.ts` (modify)
  - Endpoint: EP-31, EP-32
  - Dependency: B1-EXT.9
  - Estimate: small
  - Done: API client functions cho forgot/reset password

- [x] **B1-EXT.12** — FE: `ForgotPassword.tsx` gọi API thật, `ResetPassword.tsx` trang mới
  - Files: `Frontend/src/app/pages/ForgotPassword.tsx` (modify), `Frontend/src/app/pages/ResetPassword.tsx` (new), `Frontend/src/app/routes.tsx` (modify)
  - Dependency: B1-EXT.11
  - Estimate: medium
  - Done: ForgotPassword calls forgotPassword() API; ResetPassword receives token from URL param; Route `/reset-password` added

**Password Reset Total: 12 tasks** | **Status:** completed on 2026-05-04 | **PR:** `fix/00020-b1-password-reset-endpoints` → main (PR #20)
...
| `src/auth/models.py` | EP 1–7 | A→B1 |
| `src/itineraries/models/trip.py` | EP 8–20, 32 | A→B2 |
| `src/places/models.py` | EP 21–27, 30 | A→B3 |
| `src/itineraries/models/extras.py` | EP 14, 15, 19, 20 | A→B2 |
| `src/auth/schemas.py` | EP 1–4 | B1 |
| `src/itineraries/schemas.py` | EP 8–20, 32 | B2 |
| `src/places/schemas.py` | EP 21–27, 30 | B3 |
| `src/shared/service.py` | all services | A |
| `src/auth/repository.py` | EP 1–7 | B1 |
| `src/itineraries/repository.py` | EP 8–20, 32, 15 | B2 |
| `src/places/repository.py` | EP 21–27, 30 | B3 |
| `src/auth/service.py` | EP 1–4 | B1 |
| `src/auth/profile_service.py` | EP 5–7 | B1 |
| `src/itineraries/service.py` | EP 8–20, 32 | B2 |
| `src/places/service.py` | EP 21–27, 30 | B3 |
| `src/auth/router.py` | EP 1–4 | B1 |
| `src/itineraries/router.py` | EP 8–20, 32 | B2 |
| `src/places/router.py` | EP 21–27 | B3 |
| `src/itineraries/router.py` (mở rộng) | EP 28–31, 33 | C |
...
> **Tuần 6–7** | **Dependencies:** Phase A + B2 + B3 merged | **Kết quả:** 5 endpoints (EP 28–30, EP-33 rate-limit, EP-35 chat-history)
> **Test verify:** AI generate trip thật → chat multi-turn → suggest alternatives → rate limit triggerered

### C0: Chat Models + Migration (prerequisite for C2/C3)

- [ ] **C0.1** — `src/itineraries/models/chat.py` — ChatSession + ChatMessage tables
  - Files: `src/itineraries/models/chat.py` (new)
  - Endpoint: EP-35 (chat-history)
  - Dependency: A2.2
  - Estimate: medium
  - Details: `ChatSession` (id, trip_id, user_id, thread_id, status, created_at, updated_at) + `ChatMessage` (id, session_id, role, content, proposed_operations, requires_confirmation, created_at). Ref: [09_database_design.md §2.15-2.16](09_database_design.md)

- [ ] **C0.2** — `src/itineraries/models/__init__.py` — Export ChatSession, ChatMessage
  - Files: `src/itineraries/models/__init__.py` (modify)
  - Dependency: C0.1
  - Estimate: small

- [ ] **C0.3** — Alembic migration `0004`: create chat_sessions + chat_messages tables
  - Files: `alembic/versions/202605xx_0004_chat_tables.py`
  - Dependency: C0.1, C0.2
  - Estimate: medium
  - Details: Tables chat_sessions (FK trips, users) + chat_messages (FK chat_sessions) with indexes per [09_database_design.md §5.1](09_database_design.md)

- [ ] **C0.4** — `src/itineraries/repository.py` (mở rộng) — ChatMessageRepository methods
  - Files: `src/itineraries/repository.py` (modify)
  - Endpoint: EP-35
  - Dependency: A3.1, C0.1
  - Estimate: small
  - Details: `ChatMessageRepository`: `list_by_session()`, `create()`, `get_session_by_trip()`

### C1: Itinerary Generator (Direct Pipeline, không qua Supervisor)

- [ ] **C1.1** — `src/itineraries/pipeline.py` + folder structure
  - Files: `src/itineraries/pipeline.py`, `src/itineraries/prompts/__init__.py`, `src/itineraries/tools/__init__.py`
  - Endpoint: N/A
  - Dependency: Phase A
  - Estimate: small

- [ ] **C1.2** — `src/itineraries/pipeline.py` (~120 lines)
  - Files: `src/itineraries/pipeline.py`
  - Endpoint: EP 8
...
- [ ] **C1.3** — `src/itineraries/prompts/itinerary_prompt.py` (~80 lines)
  - Files: `src/itineraries/prompts/itinerary_prompt.py`
  - Endpoint: EP 8
  - Dependency: C1.1
  - Estimate: medium
  - Details: System prompt + few-shot examples cho Gemini 2.5 Flash

- [ ] **C1.4** — Integrate pipeline into `itineraries/service.py` `generate()`
  - Files: `src/itineraries/service.py` (modify)
  - Endpoint: EP 8
...
- [ ] **C2.1** — `src/itineraries/companion.py` (~130 lines)
  - Files: `src/itineraries/companion.py`
  - Endpoint: EP 28, 29
  - Dependency: C1.1
  - Estimate: large
...
- [ ] **C2.2** — `src/itineraries/tools/trip_tools.py` (~100 lines)
  - Files: `src/itineraries/tools/trip_tools.py`
  - Endpoint: EP 28, 29
  - Dependency: C2.1
  - Estimate: medium
...
- [ ] **C2.3** — `src/itineraries/companion.py` (mở rộng) (~80 lines)
  - Files: `src/itineraries/companion.py` (modify)
...
- [ ] **C2.4** — `src/itineraries/prompts/companion_prompt.py` (~60 lines)
  - Files: `src/itineraries/prompts/companion_prompt.py`
  - Endpoint: EP 28, 29
  - Dependency: C1.1
  - Estimate: small
...
- [ ] **C3.1** — `src/itineraries/router.py` (mở rộng) (~120 lines)
  - Files: `src/itineraries/router.py` (modify)
...
- [ ] **C3.3** — Wire agent/ws routers + update dependencies.py
  - Files: `src/main.py` (modify), `src/core/dependencies.py` (modify)
...
- [ ] **C5.1** — `src/itineraries/supervisor.py` (~80 lines)
  - Files: `src/itineraries/supervisor.py`
...
- [ ] **C5.2** — `src/itineraries/schemas.py` (mở rộng) — Supervisor schemas
  - Files: `src/itineraries/schemas.py` (modify)
  - Dependency: C5.1
  - Estimate: small
  - Details: `TravelAgentState(TypedDict)`, `AgentIntent(Enum)`, `SupervisorResponse(BaseModel)`

- [ ] **C5.3** — Wire Supervisor into itineraries router + dependencies
  - Files: `src/itineraries/router.py` (modify), `src/core/dependencies.py` (modify)
...
- [ ] **C6.1** — `src/itineraries/analytics_pipeline.py` (~100 lines)
  - Files: `src/itineraries/analytics_pipeline.py`
  - Endpoint: EP 34
...
- [ ] **C6.2** — `src/itineraries/prompts/analytics_prompts.py` (~40 lines)
  - Files: `src/itineraries/prompts/analytics_prompts.py`
  - Endpoint: EP 34
  - Dependency: C1.1
  - Estimate: small
...
- [ ] **C6.3** — Add EP-34 `POST /agent/analytics` to router
  - Files: `src/itineraries/router.py` (modify), `src/itineraries/schemas.py` (modify)
...
- [ ] **D2.3** — Docker Compose full stack verify
  - Dependency: All phases merged
  - Estimate: medium
  - Test: `docker compose up --build` → api + db + redis healthy → Swagger UI → all 34 core endpoints render; EP-36 renders only if analytics flag enabled
...
| `src/itineraries/pipeline.py` | EP 8 | C |
| `src/itineraries/companion.py` | EP 28, 29 | C |
| `src/itineraries/tools/trip_tools.py` | EP 28, 29 | C |
| `src/itineraries/prompts/itinerary_prompt.py` | EP 8 | C |
| `src/itineraries/prompts/companion_prompt.py` | EP 28, 29 | C |
| `src/itineraries/router.py` (mở rộng) | EP 28, 30, 31, 33 | C |
| `src/itineraries/router.py` (mở rộng) | EP 29 (WebSocket) | C |
...
>
> **Decision lock v5.2 (2026-05-04):** Password reset endpoints (EP-31 forgot-password, EP-32
> reset-password) đã hoàn thành trong PR #20. EP numbering updated: EP-31/32 code = forgot/reset-password;
> Phase C endpoints renumbered: EP-33 rate-limit-status, EP-34 claim, EP-35 chat-history, EP-36 analytics.
> Tổng core: 34 endpoints. Phase C thêm C0 (chat models/migration) prerequisite tasks.

| Phase | Total Tasks | Done | In Progress | Remaining | % Complete | PR |
|-------|------------|------|-------------|-----------|------------|-----|
| **A: Foundation** | 28 | 28 | 0 | 0 | 100% | #1 |
| **B1: Auth+Users** | 25 | 25 | 0 | 0 | 100% | #2 + #20 |
| **B2: Itineraries** | 16 | 16 | 0 | 0 | 100% | #3 |
| **B3: Places** | 10 | 10 | 0 | 0 | 100% | #4 |
| **C: AI Agent** | 26 | 0 | 0 | 26 | 0% | — |
| **D: ETL+Integration** | 10 | 7 | 0 | 3 | 70% | #5 + fix pending |
| **TOTAL** | **115** | **86** | **0** | **29** | **75%** |

> Update bảng này mỗi khi check task `[x]`.

---

> **Batch 2 status:** Phase B3 done, Phase D1 infrastructure + hotfix done, Password reset done (PR #20), Phase C AI still pending.
> **Grand Total:** 115 tracked tasks covering all 34 core endpoints + optional EP-36 + ETL pipeline.

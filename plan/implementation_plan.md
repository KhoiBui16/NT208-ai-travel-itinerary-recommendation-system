...
> **Version:** 5.2 (2026-05-04) — EP numbering update after password reset (PR #20), chat models tasks added
> **Quyết định đã chốt:** Integer ID ✅ | Redis rate limit ✅ | Goong key placeholder ✅ | CRUD trước, AI sau ✅ | 34 core endpoints + EP-36 optional ✅
> **New features (v4.0):** Guest Claim Trip ✅ | 5 Active Trips Limit ✅ | Share Read-Only ✅ | Chat History ✅
> **New features (v5.2):** Password Reset (EP-31 forgot-password, EP-32 reset-password) ✅ | Email service (aiosmtplib + console fallback) ✅
...
> **Total endpoints:** 34 core (27 gốc + 3 AI + EP-31 forgot-password + EP-32 reset-password + EP-33 rate-limit + EP-35 chat-history). EP-36 Analytics là optional/MVP2+ feature flag.
> **EP numbering update (v5.2):** EP-31/32 code = forgot/reset-password (was rate-limit-status/claim in v4.1). Phase C renumbered: EP-33 rate-limit-status, EP-34 claim, EP-35 chat-history, EP-36 analytics.
...
### Nhóm Auth mới (2 endpoints — PR #20)

| # | Method | Endpoint | Router fn | Service fn | Repo fn | Auth |
|---|--------|----------|-----------|------------|---------|------|
| **31** | **POST** | **`/auth/forgot-password`** | **`forgot_password()`** | **`auth_svc.forgot_password()`** | **`user_repo.get_by_email()`** | **❌ Public** |
| **32** | **POST** | **`/auth/reset-password`** | **`reset_password()`** | **`auth_svc.reset_password()`** | **`user_repo.get_by_reset_token_hash()`** | **❌ Public** |

### Nhóm AI/Suggestion (5 endpoints core + 1 optional)
...
| **33** | **GET** | **`/agent/rate-limit-status`** | **`rate_status()`** | **`limiter.remaining()`** | — | **🔒** |
| **34** | **POST** | **`/itineraries/{id}/claim`** | **`claim_trip()`** | **`itin_svc.claim()`** | **`guest_claim_repo.verify_and_consume()`** | **🔒 + claimToken** |
| **35** | **GET** | **`/agent/chat-history/{trip_id}`** | **`get_history()`** | **`companion_svc.get_history()`** | **`chat_message_repo.list()`** | **🔒 owner** |

**Optional/MVP2+ endpoint:** EP-36 `POST /agent/analytics` chỉ bật khi có read-only DB role, SQL allowlist, query checker, max rows, audit log và user-scope enforced. Không tính vào Definition of Done MVP2 core.

> **EP numbering note (v5.2):** EP-31/32 = forgot/reset-password (code reality, PR #20). Phase C endpoints renumbered from v4.1: EP-33 rate-limit-status (was EP-31), EP-34 claim (was EP-32), EP-35 chat-history (was EP-33), EP-36 analytics (was EP-34).

...
> **Domain migration note (2026-05-05):** Backend `src/` đã refactor từ by-type folders sang by-domain folders (xem PRs #34–#38). Phase A–B3 file paths dưới đây ghi bằng path cũ vì là historical specification; code thực tế giờ nằm trong `src/auth/`, `src/itineraries/`, `src/places/`, `src/shared/`, `src/core/`. Phase C planned paths đã update sang domain convention.
...
- **Quy tắc:** Code trong `src/itineraries/` KHÔNG import trực tiếp `src/itineraries/tools/` từ router/service cấp cao. Chỉ import `pipeline`, `companion`, `suggestion_service`, hoặc `supervisor` theo đúng scope.

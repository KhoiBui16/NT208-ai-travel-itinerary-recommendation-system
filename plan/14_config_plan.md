...
ai:
  ...
  enable_analytics: false               # EP-36 optional/MVP2+

# === Security Tokens ===
security_tokens:
  claim_token_expire_hours: 24
  share_token_expire_days: 30           # 30 ngày mặc định; 0 = không hết hạn
  password_reset_token_expire_hours: 1  # 1 giờ mặc định cho reset token
  store_raw_tokens: false               # chỉ lưu hash token

# === Email (SMTP) ===
email:
  smtp_host: ""                         # Trống = console fallback (log reset link ra stdout)
  smtp_port: 587
  smtp_username: ""
  smtp_password: ""                     # SecretStr — CHỈ set qua .env, không qua config.yaml
...
| **AI** | | | | | | |
| 19 | `GEMINI_API_KEY` | str | `""` (empty) | `.env` | Gemini API access. **Optional** — BE chạy CRUD không cần key. Khi gọi AI generate/chat, nếu key thiếu → 503. Không crash startup | `AIza...` |
...
| 24f | `SHARE_TOKEN_EXPIRE_DAYS` | int | 30 | `config.yaml` | 30 ngày mặc định; 0 = không hết hạn. Code default 30, plan cũ ghi 0 | `30` |
| 24g | `PASSWORD_RESET_TOKEN_EXPIRE_HOURS` | int | 1 | `config.yaml` | Password reset token hết hạn sau 1 giờ (EP-31 forgot-password) | `1` |
| **Email (SMTP)** | | | | | | |
| 24h | `SMTP_HOST` | str | `""` | `config.yaml` | SMTP server host. Trống = console fallback (log reset link ra stdout) | `smtp.gmail.com` |
| 24i | `SMTP_PORT` | int | 587 | `config.yaml` | SMTP server port | `587` |
| 24j | `SMTP_USERNAME` | str | `""` | `.env` | SMTP login username | `user@gmail.com` |
| 24k | `SMTP_PASSWORD` | SecretStr | `""` | `.env` | SMTP login password. **SecretStr** — chỉ set qua `.env` | `***` |
| 24l | `EMAIL_FROM_ADDRESS` | str | `noreply@dulichviet.local` | `config.yaml` | From address cho email gửi đi | `noreply@...` |
| **Maps** | | | | | | |
...
class AppSettings(BaseSettings):
    ...
    # --- AI ---
    GEMINI_API_KEY: str = ""  # Optional — BE CRUD runs without it; AI calls return 503 if empty
    ...
    PASSWORD_RESET_TOKEN_EXPIRE_HOURS: int = _yaml.get("security_tokens", {}).get(
        "password_reset_token_expire_hours", 1
    )

    # --- Email (SMTP) ---
    SMTP_HOST: str = _yaml.get("email", {}).get("smtp_host", "")
    SMTP_PORT: int = _yaml.get("email", {}).get("smtp_port", 587)
    SMTP_USERNAME: str = _yaml.get("email", {}).get("smtp_username", "")
    smtp_password: SecretStr = SecretStr("")  # CHỈ set qua .env
    EMAIL_FROM_ADDRESS: str = _yaml.get("email", {}).get(
        "email_from_address", "noreply@dulichviet.local"
    )
...
| `ENABLE_ANALYTICS` + `ANALYTICS_DATABASE_URL` | 04, 12, 16 | Optional EP-36 Text-to-SQL |
| `CLAIM_TOKEN_EXPIRE_HOURS` | 03, 09, 12 | Guest claim security |
| `SHARE_TOKEN_EXPIRE_DAYS` | 03, 09, 12 | Share link expiry |
| `PASSWORD_RESET_TOKEN_EXPIRE_HOURS` | 03, 09, 12 | Password reset token expiry (EP-31) |
| `SMTP_*` + `EMAIL_FROM_ADDRESS` | 03 (email service), 12 (EP-31/32) | EmailService — SMTP mode or console fallback |

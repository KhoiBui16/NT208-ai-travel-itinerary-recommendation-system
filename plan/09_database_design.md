...
> MVP2 core cần thêm `guest_claim_tokens`, `chat_sessions`, `chat_messages`. `EP-36 Analytics`
...
>
> **Decision lock v5.1 (2026-05-04):** Bảng `users` thêm 2 column cho password reset flow
> (EP-31 forgot-password, EP-32 reset-password): `password_reset_token_hash`, `password_reset_expires_at`.
> Migration 0003 đã tạo trong PR #20. Chat tables (2.15–2.16) pending Phase C migration.
...
| `password_reset_token_hash` | `String(255)` | NULL | Hash của reset token (EP-31 forgot-password). NULL = không có pending reset. Chỉ lưu hash, KHÔNG lưu raw token |
| `password_reset_expires_at` | `DateTime(tz)` | NULL | Thời điểm reset token hết hạn. Cùng lúc với token_hash: cả 2 NULL hoặc cả 2 NOT NULL |
...
class User(Base):
    ...
    password_reset_token_hash = mapped_column(String(255), nullable=True)
    password_reset_expires_at = mapped_column(DateTime(timezone=True), nullable=True)
...
> **Note (2026-05-04):** Bảng `chat_sessions` và `chat_messages` (2.15–2.16) chưa có Alembic migration
> trong code hiện tại. Sẽ được tạo khi bắt đầu Phase C (AI Agent). Cần thêm task tạo migration riêng
> trong `plan/15_todo_checklist.md` Phase C.

> **Note (2026-05-04):** Bảng `users` đã thêm 2 column mới (PR #20 / migration 0003):
> `password_reset_token_hash` (String 255, NULL) và `password_reset_expires_at` (DateTime tz, NULL).
> Đây là support cho EP-31 forgot-password và EP-32 reset-password. Column này chỉ lưu hash,
> tương tự refresh/share/claim tokens.

...
| 00138   | D     | `docs/00138-d-source-docs-sync` | Re-check Backend/Frontend/docs/README inventory, fix README Mermaid render labels, sync current source docs, and restore AI rate-limit fail-closed config invariant | review_ready | Route introspection: 41 `/api/v1` + 1 `/img`; collect-only: 194 unit + 79 integration; FE inventory: 18 e2e specs / 37 tests; Mermaid CLI render pass for README + staging guide; `uv run pytest tests/unit/test_config.py tests/unit/test_rate_limiter.py -q` = 15 passed | pending |

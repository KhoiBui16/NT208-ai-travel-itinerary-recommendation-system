...
PR này hoàn tất phase `00060E` trên branch docs-only trước khi bắt đầu `C3A`: sửa lỗi render Mermaid ERD trong `README.md`, thêm phần giải thích cách đọc ERD cho reviewer, re-check active docs/source sync sau `00060D-FIX`, và cập nhật report index tương ứng.
...
- phần giải thích ERD xuất hiện ngay sau block `### 5.1 ERD — Mermaid`
...
- Mục giải thích ERD được giữ ngắn để README vẫn là entrypoint gọn, không biến thành phase report dài.

...
| Verified 2026-06-24 | Production build pass qua `npm run build`; CI `frontend-e2e` green trên PR #109; 14 spec files Playwright ở `tests/e2e/` (cần BE chạy trên `localhost:8000`) |
...
- `npm run test:e2e`: `14` spec files ở `tests/e2e/` (cần BE chạy trên `localhost:8000`); CI `frontend-e2e` green trên PR #109. Số case chính xác chạy đủ trên CI.

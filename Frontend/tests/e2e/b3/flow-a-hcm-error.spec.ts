...
test("Flow A: TP.HCM generate shows error (422 data missing)", async ({ page }) => {
  ...
  await page.screenshot({ path: "tests/e2e/b3/screenshots/flow-a-01-create-trip-loaded.png", fullPage: true });

  // 2. Type TP.HCM destination
  const destInput = page.locator('input').first();
  await destInput.fill("TP. Hồ Chí Minh");
  await page.waitForTimeout(600);
  await page.screenshot({ path: "tests/e2e/b3/screenshots/flow-a-02-destination-filled.png", fullPage: true });
  ...
  await page.waitForTimeout(600);
  await page.screenshot({ path: "tests/e2e/b3/screenshots/flow-a-03-calendar-open.png", fullPage: true });
  ...
  if (dayCount >= 2) {
    ...
    await dayBtns.nth(2).click(); // pick 3rd available day as end date
    await page.waitForTimeout(300);
  }

  await page.screenshot({ path: "tests/e2e/b3/screenshots/flow-a-04-dates-selected.png", fullPage: true });
  ...
  await page.screenshot({ path: "tests/e2e/b3/screenshots/flow-a-05-after-calendar.png", fullPage: true });
  ...
  await page.screenshot({ path: "tests/e2e/b3/screenshots/flow-a-06-after-submit.png", fullPage: true });

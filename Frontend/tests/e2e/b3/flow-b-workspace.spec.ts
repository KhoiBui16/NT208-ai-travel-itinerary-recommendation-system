...
test("Flow B: Login and open TripWorkspace for Hà Nội trip_id=235", async ({ page }) => {
  ...
  await page.screenshot({ path: "tests/e2e/b3/screenshots/flow-b-01-login-page.png", fullPage: true });
  ...
  await page.screenshot({ path: "tests/e2e/b3/screenshots/flow-b-02-after-login.png", fullPage: true });
  console.log("After login URL:", page.url());

  // 2. Navigate to TripWorkspace
  await page.goto(`http://127.0.0.1:5173/trip-workspace?tripId=${TRIP_ID}`);
  await page.waitForTimeout(3000);
  await page.screenshot({ path: "tests/e2e/b3/screenshots/flow-b-03-workspace-loaded.png", fullPage: true });
  ...
  if (chatVisible) {
    await page.screenshot({ path: "tests/e2e/b3/screenshots/flow-b-04-floating-chat-visible.png", fullPage: true });
  }

  // 4. Scroll to see full workspace
  await page.evaluate(() => window.scrollTo(0, 500));
  await page.waitForTimeout(500);
  await page.screenshot({ path: "tests/e2e/b3/screenshots/flow-b-05-workspace-scrolled.png", fullPage: true });

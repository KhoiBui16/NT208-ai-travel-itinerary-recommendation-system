...
test("Flow C: Create Trip page observation - date picker and UI state", async ({ page }) => {
  ...
  await page.screenshot({ path: "tests/e2e/b3/screenshots/flow-c-01-create-trip-full.png", fullPage: true });

  // Observe destination suggestions
  const destInput = page.locator('input').first();
  await destInput.fill("Hà");
  await page.waitForTimeout(500);
  await page.screenshot({ path: "tests/e2e/b3/screenshots/flow-c-02-destination-suggestions.png", fullPage: true });
  ...
  await page.waitForTimeout(500);
  await page.screenshot({ path: "tests/e2e/b3/screenshots/flow-c-03-tp-suggestions.png", fullPage: true });
  ...
  if (await calendarBtn.isVisible({ timeout: 1000 }).catch(() => false)) {
    ...
    await page.waitForTimeout(500);
    await page.screenshot({ path: "tests/e2e/b3/screenshots/flow-c-04-calendar-open.png", fullPage: true });
